...
%--> lda_batch('dataset1.csv',200,0.01)
function lda_batch(file,num_iterations,learning_rate)
...
for i=1:num_iterations             %the biggest for loop until convergence
    ...
    for j=1:ds1size(1)              % it is the number of rows
        ...
        for m=1:num_attr              % number of attributes
            ...
            %disp ('temp1')                  % temp 1 is the label
            %disp (temp1);
            temp2=dataset2cell(ds1(j,m));
            temp2=cell2mat(temp2(2));           % temp 2 is the value of each attribute
            %disp ('temp2')                         
            %disp (temp2);
